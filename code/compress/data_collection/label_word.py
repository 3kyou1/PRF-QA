...
def split_text(input_string, ignore_tokens=default_ignore_tokens):
    if is_chinese(input_string):
        return chinese_split(input_string, ignore_tokens)
    else:
        return english_split(input_string, ignore_tokens)

def english_split(input_string, ignore_tokens=default_ignore_tokens):
...
def chinese_split(input_string, ignore_tokens=default_ignore_tokens):
    doc = jieba.cut(input_string, cut_all=False)
    word_list = []
    for word in doc:
...
def configure_parser():
    ...
    parser.add_argument(
        "--dataset_name", help="dataset used to compress", default="stackexchange"
    )
...
if __name__ == '__main__':
    ...
    with open(args.load_prompt_from, 'r') as f:
        data_comp = json.load(f)
    
    origins, comps = [], []

    for i, sample in data_comp.items():
    ...
    for chunk_idx, (origin, comp) in tqdm(enumerate(zip(origins, comps))):
        num_sample += 1
        origin_tokens = split_text(origin)
        comp_tokens = split_text(comp)
