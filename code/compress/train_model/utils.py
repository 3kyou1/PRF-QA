...
class TokenClfDataset(Dataset):
    ...
    def split_text(self,input_string):
        if self.is_chinese(input_string):
            return self.chinese_split(input_string,)
        else:
            return self.english_split(input_string)

    def english_split(self,input_string):
        doc = self.nlp(input_string)
        word_list = []
        for word in doc:
            if word.lemma_ not in self.ignore_tokens:
                word_list.append(word.lemma_)
        return word_list

    def chinese_split(self,input_string):
